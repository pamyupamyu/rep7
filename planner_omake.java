...
public class planner_omake {
 Vector operators;
 Random rand;
 Vector plan;
	static boolean poada;

 public static void main(String argv[]){
  (new planner_omake()).start();
 }
 ...
 public void start(){
  ...
  poada = true;
   ...
   initOperators();
   Vector goalList2     = initGoalList();
   Vector initialState2 = initInitialState();

   Hashtable theBinding2 = new Hashtable();
   plan = new Vector();
   planning(goalList2,initialState2,theBinding2);

   //System.out.println("***** This is a plan latter half! *****");
    for(int i = 0 ; i < plan.size() ; i++){
     Operator op = (Operator)plan.elementAt(i);
     System.out.println((op.instantiate(theBinding2)).name);
    }
  }
